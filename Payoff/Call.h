...
/// Call est une classe fille de Payoff.
///
class Call : public Payoff{
    public:
    // Constructeurs
    /// @brief Constructeur par défaut d'une classe Call. Les valeurs des paramètres sont initialisées avec
    /// celles fournies dans l'énoncé.
    Call() : Payoff(100, 0.1, 0.1, 1, 300, 1000, 1000){;}
    /// @brief le constructeur avec paramètres d'une classe Call
    Call(double K, double r, double sigma, double T, double L, int N, int M): Payoff(K, r, sigma, T, L, N, M){;}
    ...
    /// lorsque s=L
    /// @return La valeur du payoff à l'instant t, en s=L
    double C_L(double t) const { return K_*exp(-r_*(t-T_));}

    /// @param t un flottant 
    /// @return la valeur du payoff en s=0 (=0 pour un put)
    double C_s0(double t) const {return 0;}
...
class Call : public Payoff{
    ...
    /// celles fournies dans l'énoncé.
    Call() : Payoff(100, 0.1, 0.1, 1, 300, 1000, 1000){;}
    /// @brief le constructeur d'une classe Call
    Call(double K, double r, double sigma, double T, double L, int N, int M): Payoff(K, r, sigma, T, L, N, M){;}
    ...
    /// @return La valeur associée du payoff à l'instant ttilde 
    double C_stildeN(double ttilde) const { return get_K()*exp(2*get_r()*ttilde);}

    /// @param ttilde un flottant 
    /// @return La valeur du payoff associé à ttilde lorsque stilde=-infini (vaut toujours 0 pour un call)
    double C_stilde0(double ttilde) const {return 0;}

    /// @param stilde un flottant 
    /// @return La valeur du payoff associé à stilde lorsque ttilde = 0
    double C_ttilde0(double stilde) const{
        if(stilde > 0){
            return get_K()*(exp(stilde)-1);
        }
        return 0;
    }
